...
public class Polygon extends Geometry {
    ...
    public Polygon(Color emission, Material material, Point3D... vertices) {
        ...
        // (no need for more tests for a Triangle, could jump to set bounding box)
    ...
    public Polygon(Color emission, Point3D... vertices) {
        this(emission, Material.DEFAULT, vertices);
    }
    ...
    public List<GeoPoint> findIntersections(Ray ray, double maxDistance) {
        ...
        // let's say i = 0;
        result = alignZero(ray.get_direction().dotProduct(normals[0])); // v ∙ N1

        last_sign = result >= 0 ? result == 0 ? 0 : 1 : -1;  // result of v ∙ Ni = 0/+/- (value doesn't matter, only if it's zero )
        for (i = 1; i < n; i++) {
            result = alignZero(ray.get_direction().dotProduct(normals[i]));
            sign = result >= 0 ? result == 0 ? 0 : 1 : -1;  // v ∙ Ni = 0/+/-
            if (sign != last_sign || sign == 0) // not all signs are same. or there is a result of 0
                return null;
            // if v ∙ Ni = 0 it means that the ray is intersect the edge(tzelah) or the continuation of the edge of polygon which belong to Ni
