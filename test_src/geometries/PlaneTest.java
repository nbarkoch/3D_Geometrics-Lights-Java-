...
class PlaneTest {
    ...
    public void testFindIntersections() {
        ...
        assertEquals( List.of(new Point3D(1.5, 0.75, 1.375)),
                plane.findIntersections(new Ray(new Point3D(1, 1, 1), new Vector(4, -2, 3))),"Ray's line should be in plane");

        // TC02: Ray does not intersect the plane (0 points)
        assertNull(plane.findIntersections(new Ray(new Point3D(1, 1, 1), new Vector(4, 2, 3))), "Ray's line out of plane");

        // =============== Boundary Values Tests ==================
        Plane plane1 = new Plane(new Point3D(2, 0, 2), new Point3D(0, 0, 2), new Point3D(0, 0, 0));
        // **** Group: Ray is parallel to the plane
        // TC11: the ray included in the plane
        assertNull(plane1.findIntersections(new Ray(new Point3D(0.5, 0, 1), new Vector(2, 0, 0))), "Ray's line shouldn't be in plane");
        ...
        assertNull(plane1.findIntersections(new Ray(new Point3D(0.5, 0, 1), new Vector(0.0, -1.0, 0.0))), "Ray's line shouldn't be in plane");

        // TC15: case which P0 is after the plane
        assertNull(plane1.findIntersections(new Ray(new Point3D(0.5, -2, 1), new Vector(0.0, -1.0, 0.0))), "Ray's line shouldn't be in plane");

        //**** Group: Special cases
        // TC16: Ray is neither orthogonal nor parallel to the plane and begins at the plane
        // (P0 is in the plane, but not the ray)
        assertNull(plane1.findIntersections(new Ray(new Point3D(0.5, 0, 1), new Vector(1, -1.25, -0.3))), "Ray's line shouldn't be in plane");
        ...
        assertNull(plane1.findIntersections(new Ray(new Point3D(2.0, 0.0, 2.0), new Vector(1, -1.25, -0.3))), "Ray's line shouldn't be in plane");
    }
}